...
void ModoTrabajoB(void);
void ModoTrabajo(void);
# 2 "ContadoraCanicas.c" 2
...
uint8_t counter = 0;
uint8_t Number = 0;
unsigned int ContadorCanicasGlobal = 0;
uint8_t CanicasActuales = 0;
uint8_t CajasCompletas = 0;

enum ContadorCanicas{Paro,Arranque,Configuracion};
enum ContadorCanicas Modo;
     ...
     switch(Modo)
     {
      ...
      case Arranque:
       ...
       while(LatchB != 1)
       {
        ...
        {
         case MoverCaja:
          digitalWrite(Banda,1);
          digitalWrite(MotorCanicas,0);
          while(LatchC != 1)
          {
           Contador(0x15);
           if(digitalRead(SensorCajas) == 1)
           {
            delay(250);
            if(digitalRead(SensorCajas) == 1)
            {
             LatchC = 1;
             digitalWrite(Banda,0);
             digitalWrite(MotorCanicas,0);
            }
           }
           Tecla = KeypadRead();
         ...
         break;
         case ContarCanicas:
          digitalWrite(MotorCanicas,1);
          digitalWrite(Banda,0);
          lcdClear();
          while(LatchC != 1)
          {
           CanicasActuales = Contador(0x10);
           Tecla = KeypadRead();
...
void IOConfiguration(void)
{
 ...
 pinMode(Banda,0);
 pinMode(MotorCanicas,0);
 pinMode(IndicadorCanicas,0);
 pinMode(Alarma,0);
 pinMode(SensorCanicas,1);
 pinMode(SensorCajas,1);

    digitalWrite(MotorCanicas,0);
    digitalWrite(Banda,0);
 digitalWrite(Alarma,0);
 digitalWrite(IndicadorCanicas,0);
}
